defmodule TugasWeb.TodoLiveTest do
  ...
  test "desktop todo team log page shows entity activity", %{conn: conn} do
    ...
    {:ok, _} = Tugas.Todos.create_todo(scope, %{title: "Logged task"})
  ...
  test "clicking a deleted todo's log entry flashes a notice instead of navigating", %{
    ...
    {:ok, todo} = Tugas.Todos.create_todo(scope, %{title: "Temp task"})
    {:ok, _} = Tugas.Todos.delete_todo(scope, todo)
  ...
  test "mobile todo team log page loads", %{conn: conn} do
    ...
    {:ok, _} = Tugas.Todos.create_todo(scope, %{title: "Mobile log entry"})
  ...
  test "completed todo only allows reopen on desktop", %{conn: conn} do
    ...
    {:ok, todo} = Tugas.Todos.create_todo(scope, %{title: "Restock pantry"})
  ...
  test "completing an open todo plays animation and keeps the row", %{conn: conn} do
    ...
    {:ok, todo} = Tugas.Todos.create_todo(scope, %{title: "Ship labels"})
  ...
  test "member creates and deletes an open todo on desktop", %{conn: conn} do
    ...
    {:ok, [todo]} = Tugas.Todos.list_todos(scope)
  ...
  test "todo created by one user appears for another team member", %{conn: conn} do
    ...
    {:ok, _} = Tugas.Todos.create_todo(creator, %{title: "Shared task"})
  ...
  test "unified list shows open and completed todos together without a filter", %{conn: conn} do
    ...
    {:ok, open_todo} = Tugas.Todos.create_todo(scope, %{title: "Still open"})
    {:ok, done_todo} = Tugas.Todos.create_todo(scope, %{title: "Already done"})
    {:ok, _} = Tugas.Todos.toggle_complete(scope, done_todo)
  ...
  test "manager sees escalate link and old todos show cancel instead of delete", %{conn: conn} do
    ...
    {:ok, fresh} = Tugas.Todos.create_todo(manager, %{title: "Fresh todo"})
    {:ok, stale} = Tugas.Todos.create_todo(manager, %{title: "Stale todo"})
  ...
  test "cancel modal cancels stale todo after note", %{conn: conn} do
    ...
    {:ok, todo} = Tugas.Todos.create_todo(scope, %{title: "Old cleanup"})
  ...
  test "escalate pre-fills obligation form from todo", %{conn: conn} do
    ...
    assert {:ok, []} = Tugas.Todos.list_todos(manager, status: :open)
  ...
  test "highlight param flashes a todo already on the first page", %{conn: conn} do
    scope = entity_scope_fixture()
    conn = log_in_user(conn, scope.user)
    {:ok, todo} = Tugas.Todos.create_todo(scope, %{title: "On page one"})
    todo_id = todo.id
  ...
  test "highlight param for a deleted todo is ignored", %{conn: conn} do
    scope = entity_scope_fixture()
    conn = log_in_user(conn, scope.user)
    {:ok, todo} = Tugas.Todos.create_todo(scope, %{title: "Gone soon"})
    {:ok, _} = Tugas.Todos.delete_todo(scope, todo)
