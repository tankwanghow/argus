defmodule TugasWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, TugasWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      TugasWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_entity" do
    ...
    test "assigns entity and role to current_scope", %{conn: conn, user: user} do
      ...
      membership = Tugas.Entities.get_membership!(user, entity)
      user_token = Accounts.generate_user_session_token(user)
      session = conn |> put_session(:user_token, user_token) |> get_session()
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        TugasWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
