defmodule TugasWeb.ViewModeControllerTest do
  use TugasWeb.ConnCase, async: true

  setup :register_and_log_in_user

  test "sets the tugas_view cookie and redirects to the local target via mode param", %{
    conn: conn
  } do
    conn = get(conn, ~p"/view-mode?#{[mode: "mobile", to: "/m/acme"]}")

    assert redirected_to(conn) == "/m/acme"
    assert conn.resp_cookies["tugas_view"].value == "mobile"
  end
  ...
  test "legacy set-view route accepts view param", %{conn: conn} do
    ...
    assert conn.resp_cookies["tugas_view"].value == "desktop"
  end
