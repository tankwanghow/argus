defmodule Tugas.Obligations.DocumentsTest do
  ...
  describe "add_document/5 and void_document/4" do
    ...
    test "uploader can delete own document within 48 hours" do
      ...
      refute Tugas.Repo.get(Tugas.Obligations.EventDocument, document.id)
    end
    ...
    test "files on old slot names are ignored for Done after slots change" do
      ...
      {:ok, _type} =
        Tugas.Obligations.update_type(manager, type, %{complete_documents: "payment_receipt"})

      synced = Tugas.Repo.get!(Tugas.Obligations.Obligation, obligation.id)
    ...
    test "document cannot be deleted after 48 hours on a live cycle" do
      ...
      assert Tugas.Repo.get!(Tugas.Obligations.EventDocument, old.id)
    end
