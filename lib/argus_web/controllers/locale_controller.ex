defmodule ArgusWeb.LocaleController do
  @moduledoc """
  Persists the logged-in user's UI locale and redirects back to the
  referring page, so `ArgusWeb.Locale` re-applies it on the next request.
  """
  ...
  def update(conn, %{"locale" => locale}) when locale in @locales do
    ...
    conn
    |> put_resp_cookie("argus_locale", locale, max_age: 60 * 60 * 24 * 365, same_site: "Lax")
  ...
  defp safe_path(referer) when is_binary(referer) do
    ...
    path = uri.path || "/"
    app_host = ArgusWeb.Endpoint.config(:url)[:host] || "localhost"
