defmodule Tugas.Duties.Duty do
  use Tugas.Schema
  import Ecto.Changeset

  alias Tugas.Accounts.User
  alias Tugas.Entities.Entity
  alias Tugas.Duties.{Collaborator, Event, Type}
  ...
  @cast_fields ~w(title duty_type_id primary_assignee_id due_by open_note someday)a
  ...
  def changeset(duty, attrs) do
    ...
    |> maybe_clear_due_by()
    |> validate_required([:title, :duty_type_id])
    |> validate_due_by()
