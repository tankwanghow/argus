defmodule TugasWeb.MobileLive.DutyForm do
  @moduledoc """
  Mobile new-duty form. Mirrors `DutyLive.Form` but renders in the
  `Layouts.mobile_app` bottom-nav shell; all logic is shared via
  `TugasWeb.DutyLive.CreateForm`.
  """
  ...
  def mount(params, _session, socket) do
    if Tugas.Authorization.can?(socket.assigns.current_scope, :create_duty) do
  ...
  def handle_event("save", %{"duty" => params}, socket) do
    CreateForm.save(socket, params, fn scope, duty ->
      ~p"/m/#{scope.entity.slug}/duties/#{duty.id}"
    end)
  end
