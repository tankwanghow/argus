defmodule TugasWeb.DutyLive.DocumentEvents do
  @moduledoc false
    ...
    for event <- @events do
      quote do
        def handle_event(unquote(event), params, socket) do
          TugasWeb.DutyLive.DocumentEvents.dispatch(
            unquote(event),
            params,
            socket,
            &reload/1
  ...
  def dispatch("open_step_files", %{"event_id" => event_id}, socket, _reload) do
    case DocumentHelpers.find_event(socket.assigns.duty.events, event_id) do
      nil ->
  ...
  def dispatch("delete_document", %{"document_id" => document_id} = params, socket, reload) do
    ...
    case DocumentHelpers.find_event_document(duty.events, event_id, document_id) do
      nil ->
        {:noreply, Phoenix.LiveView.put_flash(socket, :error, "Document not found.")}

      document ->
        case Duties.delete_document(scope, duty, document) do
          {:ok, _} ->
            {:noreply,
             socket
             |> reload.()
  ...
  def dispatch("confirm_void_document", %{"document_id" => document_id} = params, socket, reload) do
    ...
    case DocumentHelpers.find_event_document(duty.events, event_id, document_id) do
      ...
      document ->
        case Duties.void_document(scope, duty, document, %{reason: reason}) do
          {:ok, _} ->
            {:noreply,
             socket
             |> reload.()
