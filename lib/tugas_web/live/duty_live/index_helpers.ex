defmodule TugasWeb.DutyLive.IndexHelpers do
  ...
  @doc "Combined status atom for `Duties.list_duties/2`."
  ...
  def load_rows(scope, today, mine?, lifecycle, query) do
    ...
    case Duties.list_duties(scope, status: status, query: query) do
      :not_authorise -> []
      duties -> build_rows(duties, today)
    end
  end
  ...
  defp do_load_page(scope, today, status, query, sort, cursor) when sort != :urgency do
    case Duties.list_duties_page(scope,
           status: status,
           query: query,
           sort: sort,
           cursor: cursor,
  ...
  defp serve_window(scope, today, status, window_end, query, offset) do
    ranked =
      ...
      |> Enum.sort_by(fn %Duty{} = o ->
        {@urgency_rank[Urgency.classify(o.duty_type, o.due_by, today)], Date.to_iso8601(o.due_by)}
      end)
  ...
  def build_rows(duties, today) do
    ...
    Enum.map(duties, fn duty ->
      %{event_count: event_count, latest_event: latest_event} =
        Map.fetch!(summaries, duty.id)

      %{
        duty: duty,
        cycle_status: cycle_status(duty),
  ...
  def cycle_status(%Duty{completed_at: %DateTime{}}), do: :completed
  ...
  def cycle_status(%Duty{closed_at: %DateTime{}, series_ended_at: %DateTime{}}),
    do: :series_ended

  def cycle_status(%Duty{closed_at: %DateTime{}}), do: :skipped
