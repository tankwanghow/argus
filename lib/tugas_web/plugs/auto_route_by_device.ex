defmodule TugasWeb.Plugs.AutoRouteByDevice do
  @moduledoc """
  ...
  An `tugas_view=mobile|desktop` cookie short-circuits the UA check so
  ...
  defp do_route(conn) do
    cookie = conn.cookies["tugas_view"]
  ...
  defp mobile_ua?(conn), do: TugasWeb.Device.mobile_ua?(conn)
