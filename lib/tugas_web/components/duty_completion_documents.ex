defmodule TugasWeb.DutyCompletionDocuments do
  @moduledoc """
  ...
  def completion_documents(assigns) do
    {slot_rows, voided} =
      TugasWeb.DutyLive.DocumentHelpers.completion_view(
        assigns.documents,
    ...
    <section id={"#{@id_prefix}completion-docs"} class="space-y-3">
      <div :if={@slot_rows == []} class="text-sm text-base-content/50">
        This duty type has no required completion documents.
      </div>
      ...
      <ul class="divide-y divide-base-300 rounded-box border border-base-300">
        ...
        >
          <div class="flex items-center gap-x-2">
            ...
            <.doc_link
              :if={live}
              href={"/entities/#{@entity_slug}/duties/#{@duty.id}/documents/#{live.id}"}
              name={DutyDocumentRow.file_name(live)}
