defmodule TugasWeb.CoreComponents do
  @moduledoc """
  ...
  Delegates to `Tugas.Accounts.User.display_name/1`; use this in any template
  that would otherwise render a user's raw email.
  """
  def user_label(%Tugas.Accounts.User{} = user), do: Tugas.Accounts.User.display_name(user)
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(TugasWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(TugasWeb.Gettext, "errors", msg, opts)
    end
  end
