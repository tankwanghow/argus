defmodule TugasWeb.UserSessionController do
  ...
  def update_password(conn, %{"user" => user_params} = params) do
    ...
    # Normalize the params so the create/3 clause sees "identifier" (the settings
    # form sends a hidden "email" field for re-authentication purposes).
    normalized_user_params =
      case user_params do
        %{"email" => email} = p -> Map.put(p, "identifier", email)
        p -> p
      end

    conn
    |> put_session(:user_return_to, ~p"/users/settings")
    |> create(%{params | "user" => normalized_user_params}, "Password updated successfully!")
  end
