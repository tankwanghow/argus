defmodule TugasWeb.ViewModeController do
  @moduledoc """
  Pins the user's UI preference (mobile vs. desktop) to a cookie so
  `TugasWeb.Plugs.AutoRouteByDevice` respects their explicit choice
  across sessions.
  """
  ...
  def set(conn, %{"to" => to} = params) do
    ...
    if mode in ["mobile", "desktop"] and is_binary(to) do
      conn
      |> put_resp_cookie("tugas_view", mode, max_age: @cookie_max_age, same_site: "Lax")
